...
class Booking1stClass extends Simulation {
	...
	val search = exec(http("Home Page")
		...
		.exec(http("Search for Trip")
			...
			.headers(headers_4)
			.body(RawFileBody("com/gatling/tests/Booking/booking1stclass/0004_request.json")))
		.pause(6)
	...
	val book = exec(http("Select Trip")
		...
		.exec(http("Submit Booking")
			...
			.headers(headers_11)
			.body(RawFileBody("com/gatling/tests/Booking/booking1stclass/0011_request.json")))
		.pause(5)
		.exec(http("Home Page")
			.get("/index.html"))
		.pause(2)
