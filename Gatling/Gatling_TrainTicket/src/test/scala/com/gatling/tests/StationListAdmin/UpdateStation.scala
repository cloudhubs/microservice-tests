...
class UpdateStation extends Simulation {
	...
	val scn = scenario("UpdateStation")
		.exec(http("request_0")
		...
		.exec(http("request_3")
			...
			.headers(headers_3)
			.body(RawFileBody("com/gatling/tests/StationListAdmin/updatestation/0003_request.json")))
		.pause(1)
		.exec(http("request_4")
			.get("/admin_station.html")
			.headers(headers_0)
