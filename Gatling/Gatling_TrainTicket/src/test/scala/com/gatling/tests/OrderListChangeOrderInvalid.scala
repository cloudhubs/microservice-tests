...
class OrderListChangeOrderInvalid extends Simulation {
	...
	val users = scenario("Users Changing Order")
		...
		.exec(http("View Order List")
			...
			.headers(headers_6)
			.body(RawFileBody("com/gatling/tests/orderlistchangeorderinvalid/0006_request.json")),
				http("Refresh Page")
			.post("/api/v1/orderservice/order/refresh")
			.headers(headers_6)
			.body(RawFileBody("com/gatling/tests/orderlistchangeorderinvalid/0007_request.json"))))
		.pause(4)
		.exec(http("Select Change Order")
			.post("/api/v1/travelservice/trips/left")
			.headers(headers_6)
			.body(RawFileBody("com/gatling/tests/orderlistchangeorderinvalid/0008_request.json")))
		.pause(6)
		.exec(http("Confirm Rebook")
			.post("/api/v1/rebookservice/rebook")
			.headers(headers_6)
			.body(RawFileBody("com/gatling/tests/orderlistchangeorderinvalid/0009_request.json")))
		.pause(10)
		.exec(http("Home Page")
			.get("/index.html"))
		.pause(2)
	...
	setUp(
		users.inject(rampUsers(400).during(30))
