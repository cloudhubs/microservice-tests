...
class AddTrainInvalid extends Simulation {
	...
	val scn = scenario("AddTrainInvalid")
		.exec(http("request_0")
		...
		.exec(http("request_3")
			...
			.headers(headers_3)
			.body(RawFileBody("com/gatling/tests/TrainListAdmin/addtraininvalid/0003_request.json")))
		.pause(1)
		.exec(http("request_4")
			.get("/admin_train.html")
			.headers(headers_0)
